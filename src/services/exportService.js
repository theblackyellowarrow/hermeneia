...
export function generatePrintHTML(translatedPages, totalPages, direction, exportTitle, exportAuthor, exportFont, includeCover, includeLexicon, annotations) {
  ...
  if (includeCover) {
    html += `<div class="cover-page"><div class="cover-top"><span class="cover-indicator"></span><span>HERMENEIA EXPORT</span></div><div class="cover-middle"><h1 class="cover-title">${escapeHtml(exportTitle) || 'Untranslated Publication Dossier'}</h1><div class="cover-subtitle">Translation &amp; Annotation Dossier</div></div><div class="cover-bottom"><div><strong>Compiler:</strong> ${escapeHtml(exportAuthor) || 'Hermeneia Suite'}</div><div style="margin-top:4px"><strong>Methodology:</strong> OpenAI + DeepSeek Multi-Worker Pipeline</div><div style="margin-top:4px"><strong>Translation Scope:</strong> ${direction === 'RU_TO_EN' ? 'Russian to English' : 'English to Russian'}</div><div style="margin-top:8px;color:#888">dotai.org</div></div></div>`;
  }
